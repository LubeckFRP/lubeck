...
{-|
This module provides a way to write static and reactive single-page apps with /virtual-dom/ and "Lubeck.FRP".
All functions in this module are intended to be bound to main, and will take over the main thread with the GUI event loop.
...
To get interactivity, add event listeners to the generated HTHL. These can be fed back into the network using events (see "Lubeck.FRP"
for details).

The 'runApp' and 'runAppPure' functions provide an Elm-style top level loop, in which the whole page is represented by a single-page
